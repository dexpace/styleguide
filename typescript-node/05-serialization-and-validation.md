...
Serialization is where the runtime meets everything it does not control: the process environment, request and response bodies, queue messages, third-party JSON. The core guide put a zod schema at the API boundary and derived the type with `z.infer` ([core 10.7](../typescript/10-api-design.md)); this chapter extends that discipline to every edge a Node process touches, environment included (NODE-3), and fixes the wire encodings — time, null-versus-absent, money, binary — that a long-running service cannot afford to get wrong at 3am.

## What good looks like

```ts
// config.ts — process.env is parsed once, at startup, into a frozen typed config.
import {z} from 'zod';
...
export const config: Config = loadConfig(process.env); // parsed once, at module load
```

The schema runs exactly once, at startup, and a single failure lists *every* problem before the process exits (5.1) — a server never starts half-configured. The type is `z.infer` of the schema, frozen and `Readonly`, so the validator and the type cannot drift (5.2, [core 10.7](../typescript/10-api-design.md)) and nothing mutates the config after boot. `process.env` enters as the untyped thing it is and is parsed, never read field-by-field with raw string access (5.3). Defaults live in the schema, in one place; the boundary is the only place `env` is touched.

## Rules

### 5.1 — Parse `process.env` once, at startup, into a frozen typed config; fail the boot with every problem listed.

**Reasoning, step by step:**
1. The environment is the first untrusted boundary a process crosses, and it is read at the worst possible time to discover it is wrong — lazily, deep in a request, hours after deploy. So parse it *once*, at module load, through a zod schema, into a single frozen `Readonly` config object. Every other module imports that object; none reads `process.env` directly. The boundary is one place, crossed one time (NODE-3).
2. When the environment is invalid the boot *fails* — a thrown error, a non-zero exit, before the server binds a port. A service that starts half-configured does not fail now; it fails at 3am, mid-request, in a state no one designed (fail fast, NODE-3). Crashing at boot is the supervisor's cue to hold the old version.
...
```
**Enforcement:** review; one `config` module owns the single `process.env` read, schema-parsed and frozen at load; a `process.env.` access anywhere else is a finding (lint via `no-process-env` outside the config module).

### 5.2 — Every request and response body has a zod schema, and `z.infer` is its type.

**Reasoning, step by step:**
1. A request body arrives as `unknown` over the wire and must be parsed into a domain type before a handler touches it — this is [core 10.7](../typescript/10-api-design.md) and the route rule of [core 03](../typescript/03-the-type-system.md), unchanged. The Node layer only adds that *every* body qualifies: inbound request, outbound response, and the message on every queue between them (NODE-3).
...
2. In the domain, behind that boundary, use `Temporal` where the runtime provides it natively — `Temporal.Instant` for a point in time, `Temporal.PlainDate` for a timezone-free calendar date. Until `Temporal` is available across the deployed Node versions, use `date-fns`: explicit, immutable, tree-shakeable functions over a `Date`.
3. Never `moment`. It is a frozen project by its own maintainers' declaration, its objects are mutable, and it pulls a heavy bundle no service should carry. A new dependency on it fails NODE-4's justification on every count.
...
```
**Enforcement:** review; `z.iso.datetime()` on wire timestamps, never a bare epoch number; `moment` blocked at dependency review (NODE-4).
...
2. Where the extra key signals a client bug or a tampering risk — a command, a write, a money-moving request — strip is too quiet. Use `.strict()` so an unexpected key is a parse error, not a silent drop: on a write you want to *know* the caller sent a field you did not honour, before it costs an audit.
3. `.passthrough()`, which carries unknown keys through untyped, is never the silent default. It reintroduces the unvalidated baggage the schema exists to remove; permit it only at a deliberate proxy boundary, named and commented, where forwarding an opaque body is the actual job.

```ts
const Read = z.object({id: z.string()});            // strips unknown keys — forgiving read
const Command = z.object({amount: z.number()}).strict(); // unknown key = error on a write
```
**Enforcement:** review; `.strict()` on commands and writes, default strip on reads; `.passthrough()` only at a commented proxy boundary.
...
3. In production the check need not be a second pass. A schema-aware serializer — Fastify's per-route response schema — both serializes and enforces the shape in one step, so the outbound guarantee costs nothing extra on the hot path while remaining the same contract the dev-time parse asserts.

```ts
// dev/test: assert the contract before sending
const body = UserResponse.parse(assembleUser(row)); // a leaked field fails here, not in prod
// prod: the route's response schema serializes and enforces in one pass (Fastify)
app.get('/users/:id', {schema: {response: {200: UserResponseJsonSchema}}}, handler);
```
**Enforcement:** review; response bodies parsed against their schema in dev/test, or enforced by a schema-aware serializer in prod; outbound shape is checked, not assumed.
...
- Parse every boundary, crash-only boot, dependency justification: NODE-3, NODE-1, NODE-4 ([README](./README.md)). Rows parsed at the edge: [persistence](./04-persistence.md). HTTP body limits and handlers: [HTTP services](./03-http-services.md).
