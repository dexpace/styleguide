...
const server = Bun.serve({ port: 3000, fetch: handler }); // one loop, one server handle

async function shutdown(signal: NodeJS.Signals): Promise<void> {
  log.info({ signal }, 'shutdown: draining');
  controller.abort(); // stop timers, intervals, background loops at once (2.9)
  const deadline = AbortSignal.timeout(SHUTDOWN_DEADLINE_MS); // bounds the drain; the catch forces exit on a hung stop()/drain (2.6)
  try {
    server.stop();                                 // 1. stop accepting new connections (Bun.serve handle)
    await drainInFlight({ signal: deadline });     // 2. let live requests finish, or time out (2.8)
    await sql.end();                               // 3. close DB pool after its last user (typescript/13.7)
  } catch (err: unknown) {
...
2. A 30ms synchronous read looks harmless alone. Under 500 req/s it is a 30ms tax on the tail of every concurrent request, and it surfaces as event-loop lag (2.2), not a slow handler — hard to attribute after the fact. The legitimate home for `*Sync` is startup — config, a TLS cert, a path resolved *before* the server starts accepting, when there is no concurrent request to starve; once `Bun.serve()` has run, async is the only form.

```ts
import { readFileSync } from 'node:fs';
const cert = readFileSync('tls.pem');          // fine at startup, before Bun.serve()
const body = await Bun.file(path).text();      // request path: async, the loop stays free
```
...
**Reasoning, step by step:**
1. Event-loop lag — the delay between when a timer or I/O callback *should* run and when the loop gets to it — is the truest health signal a Bun service has (BUN-2). High lag means the thread is busy synchronously, and a busy thread serves no one; request latency follows lag.
2. Node measures this with `monitorEventLoopDelay()` from `node:perf_hooks`, but **Bun does not give you a verified one**: as of 1.3 the docs mark `node:perf_hooks` only 🟡 partial — "APIs are implemented, but Node.js test suite does not pass yet for this module" — and `monitorEventLoopDelay` is nowhere confirmed, so do not build a health alert on an API the runtime won't vouch for. Measure lag honestly with an interval-drift sampler instead: schedule a tick on a fixed period, compare when it *actually* fired against wall clock (the `performance` global *is* fully implemented on Bun), and the difference is the lag the loop accumulated. The default budget is **p99 lag < 50ms**; sustained breach is an alert, not a dashboard curiosity, because it predicts the timeouts users are about to see. A breach points at exactly this chapter's defects — a `*Sync` call (2.1), CPU work that belongs on a worker (2.3), a hot loop that never yields (2.7) — so the metric is how you find them in production instead of guessing.

```ts
// Bun has no verified monitorEventLoopDelay (perf_hooks 🟡 partial, 1.3); sample the loop's own scheduling drift instead.
const PERIOD_MS = 100;
...
id.unref?.(); // liveness hint only; still cleared on shutdown (2.9)
```

**Enforcement:** review; an interval-drift sampler wired at startup (Bun has no verified `monitorEventLoopDelay`), p99 exported, alert on the stated budget (BUN-2).

### 2.3 — CPU-bound work goes to Bun `Worker`s via a bounded pool.
...
2. Move that work to Bun `Worker`s (the Web `Worker` API, native on Bun) behind a hand-rolled bounded pool — Bun ships no worker-pool library in the runtime, so you own the ceiling yourself. The loop serves I/O while workers serve math, and the pool's ceiling means a burst of CPU jobs queues against a known bound instead of spawning unbounded threads (root rule 9). Size the worker count to `availableParallelism()`, not request volume — more workers than cores just thrashes the scheduler — and read it explicitly, because a container's cgroup CPU quota can differ from what the host reports, so leaving it implicit risks a pool sized for the host's cores rather than the ones the orchestrator actually grants (root rule 2). Bound the queue with an `invariant` so that at the limit you shed (2.8), not grow.

```ts
import { availableParallelism } from 'node:os';
const SIZE = availableParallelism(); // workers == cores/cgroup quota, never request volume
const MAX_QUEUE = 1_000;             // bounded; at the limit we shed (2.8), not grow
const idle = Array.from({ length: SIZE }, () => new Worker(new URL('./workers/resize.ts', import.meta.url)));
const queue: Array<{ job: unknown; resolve: (v: unknown) => void }> = [];

function run(job: unknown): Promise<unknown> {
  invariant(queue.length < MAX_QUEUE, 'cpu pool queue full'); // overflow rejects, not grows (root rule 9)
  return new Promise((resolve) => {
    const w = idle.pop();
    if (!w) return void queue.push({ job, resolve });        // no free worker: wait against the bound
    w.onmessage = (e) => { resolve(e.data); idle.push(w); const n = queue.shift(); if (n) run(n.job).then(n.resolve); };
    w.postMessage(job);
  });
}
```

**Enforcement:** review; CPU-bound spans run on a bounded Bun `Worker` pool sized from `availableParallelism()` with a queue bounded by `invariant`, never inline on the loop.
...
2. Bun exposes Web Streams (`ReadableStream`/`WritableStream`) natively, and `Bun.serve` request and response bodies *are* Web Streams — so prefer them at the HTTP edge, and async-iterate (`for await (const chunk of stream)`) to get backpressure for free, since the loop body's `await` is the pause signal. The `node:stream` API works too under Bun's compatibility layer: wire those pipelines with `stream.pipeline` (or async iterators), never a bare `source.pipe(dest)`. `pipeline` propagates errors and backpressure across every stage and destroys the whole chain on failure; a lone `.pipe()` drops errors on the floor and leaks half-open handles when one stage fails. Either path is fine; what is banned is the unbacked `.pipe()`.
...
**Reasoning, step by step:**
1. These two events fire when a fault escaped every local handler — a promise no one caught, a throw outside any `try`. By definition the process is now in a state no code anticipated. This is the crash-only policy (BUN-1): the only safe assumption about unknown state is that it is corrupt.
...
**Enforcement:** review; both handlers present, each logs with context and exits non-zero — never a `return` that resumes serving (BUN-1).
...
2. The order is fixed (the exemplar): stop accepting (`server.stop()` on the `Bun.serve` handle), drain in-flight on a deadline (2.8), then close pools and clients *after* their last user — reverse-acquisition release, mirroring [typescript/13.7](../typescript/13-resource-management.md). The drain carries an `AbortSignal.timeout`, and the forced exit on overrun is the backstop for a phase that ignores it — a hung `stop()`/`end()`; an unbounded drain just trades a fast restart for a hung one, so set the deadline below the orchestrator's grace and let cleanup win the race.
...
2. Yield between chunks so the loop can serve I/O in the gaps. `await setImmediate()` (from `node:timers/promises`, supported on Bun) returns control after pending I/O callbacks; `await Bun.sleep(0)` is the Bun-native spelling that does the same. Yield per *batch*, not per row — yielding too often is its own overhead. Pair the yield with `signal.throwIfAborted()` at the top of each chunk ([typescript/9.11](../typescript/09-concurrency.md)) so a shutdown (2.6) or client disconnect stops the loop promptly, not after the last row.
...
for (let i = 0; i < rows.length; i += BATCH) {
  ...
  await yieldToLoop();               // hand the loop back to in-flight requests (or: await Bun.sleep(0))
}
```

**Enforcement:** review; CPU-heavy loops `await setImmediate()` (or `Bun.sleep(0)`) per batch and check `throwIfAborted()`.
...
let inFlight = 0;
app.use('*', async (c, next) => {                          // Hono middleware on Bun.serve (03)
  if (inFlight >= MAX_INFLIGHT) return c.json({ error: 'overloaded' }, 503, { 'retry-after': '1' }); // shed, don't queue
  inFlight++;
  try { await next(); } finally { inFlight--; }            // decrement even if the handler throws
});
```
...
- Crash-only policy (BUN-1) and operational vs programmer errors: [typescript/08](../typescript/08-error-handling.md), [Bun README](./README.md). The Hono request lifecycle these middlewares attach to: [03 — HTTP services](./03-http-services.md); runtime and one-process-per-container: [01 — runtime and toolchain](./01-runtime-and-toolchain.md).
