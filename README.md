...
9. **Limits on everything.** All loops, queues, retries, buffers, and timeouts must have a fixed upper bound. No recursion in library code — all execution must be provably bounded. Functions must not exceed the language's documented size cap; each guide sets its own (Go 70, Kotlin 60, Python 50, TypeScript and C# 70, Ruby 25).
10. **Small functions, breathing room.** Functions do one thing. Aim well under the cap, at the target band each guide sets. Separate logical sections with blank lines. Cramped code is unreadable code. Whitespace is free — use it.
...
| Kotlin (any target) | [`kotlin/`](./kotlin/) | [Kotlin coding conventions](https://kotlinlang.org/docs/coding-conventions.html) + [Google Android Kotlin Style Guide](https://developer.android.com/kotlin/style-guide) | Platform-agnostic Kotlin: nullability, sealed hierarchies, scope functions, coroutines, structured concurrency. |
...
| Python | [`python/`](./python/) | [PEP 8](https://peps.python.org/pep-0008/) + [PEP 20](https://peps.python.org/pep-0020/) + [PEP 484/604](https://peps.python.org/pep-0484/) + [PEP 695](https://peps.python.org/pep-0695/) + [Google Python Style Guide](https://google.github.io/styleguide/pyguide.html) | Python 3.12+. Type hints + mypy strict, dataclasses + Protocols, asyncio with `TaskGroup`, Ruff for lint + format. |
| Ruby | [`ruby/`](./ruby/) | [Airbnb Ruby Style Guide](https://github.com/airbnb/ruby) + [Shopify Ruby Style Guide](https://ruby-style-guide.shopify.dev/) + [rubystyle.guide](https://rubystyle.guide/) | Ruby 4.0+. Sorbet `# typed: strict` with runtime-checked sigs, `Data.define` value objects, frozen by default, parse-don't-validate, Minitest, `rubocop-airbnb`. |
| TypeScript | [`typescript/`](./typescript/) | [Google TS Style Guide](https://google.github.io/styleguide/tsguide.html) + [ts.dev/style](https://ts.dev/style/) | TS 5.8+, erasable syntax only, gts toolchain + type-aware overlay, bun test + fast-check + expectTypeOf, zod at boundaries. |
| TypeScript on Bun | [`typescript-bun/`](./typescript-bun/) | Extends `typescript/`; [Bun docs](https://bun.com/docs) | Bun ≥ 1.3 pinned (no LTS), native TS execution + tsc gate, Hono on Bun.serve, Bun.SQL + Drizzle, bun test, JSC perf. |
...
| C# | [`csharp/`](./csharp/) | [.NET Runtime C# Coding Style](https://github.com/dotnet/runtime/blob/main/docs/coding-guidelines/coding-style.md) + [MS Learn conventions](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/coding-style/coding-conventions) | .NET 10 / C# 14, nullable reference types as law, records + functions over hierarchies, `sealed` by default, `dotnet format` + Roslyn analyzers. |
| C# on ASP.NET Core | [`csharp-aspnetcore/`](./csharp-aspnetcore/) | Extends `csharp/`; [ASP.NET Core docs](https://learn.microsoft.com/en-us/aspnet/core/) + [Framework Design Guidelines](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/) | Minimal APIs, explicit DI lifetimes, EF Core with `AsNoTracking`, System.Text.Json source-gen, OpenTelemetry, WebApplicationFactory + Testcontainers. |
...
- **[Airbnb Ruby Style Guide](https://github.com/airbnb/ruby)**, **[Shopify Ruby Style Guide](https://ruby-style-guide.shopify.dev/)**, and **[Sorbet](https://sorbet.org/)** — guard clauses, immutable value objects, parse-don't-validate, runtime-checked type signatures as the first test suite.
