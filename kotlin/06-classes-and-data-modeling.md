...
4. Variants live as nested types under the parent — the lexical grouping documents the closure: `sealed interface Result { data class Ok<T>(val value: T) : Result; data class Err(val cause: Throwable) : Result }`.
