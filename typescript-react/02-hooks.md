...
3. Push shared logic *down* into a reusable hook rather than *sideways* by duplicating effect code across components. Two components reading the same external store should both call one custom hook built on `useSyncExternalStore` — the platform primitive for subscribing a component to a mutable value outside React, which keeps the read tear-free under concurrent rendering where a hand-rolled `useEffect` + `useState` subscription does not ([react.dev: useSyncExternalStore](https://react.dev/reference/react/useSyncExternalStore)). The deduplication that core §12 wants for functions applies identically to hooks.
...
4. The one sanctioned exception is the `use` API, which is *not* a hook in this respect: it may be called inside an `if`, a loop, or after an early return. Reach for it to read a Context conditionally — `if (children == null) return null; const theme = use(ThemeContext);` — where `useContext` would force the read above the guard, and to read a promise with Suspense at the read site. `use` is the only call that may sit behind a condition; every other hook stays at the top ([react.dev: use](https://react.dev/reference/react/use)).
